...
if ($this->Context->Session->UserID > 0 && $this->Context->Session->User->Permission('PERMISSION_START_DISCUSSION')) {
	...
	echo '<button onclick="window.location.href=\''.GetUrl($this->Context->Configuration, 'post.php', 'category/', 'CategoryID', $CategoryID).'\';">'.$this->Context->GetDefinition('StartANewDiscussion').'</button>
		<h1><a href="'.GetUrl($this->Context->Configuration, 'post.php', 'category/', 'CategoryID', $CategoryID).'">'

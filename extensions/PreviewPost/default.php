...
if(in_array($Context->SelfUrl, array('post.php', 'comments.php')))
{
	...
	{
		echo(
			'<input name="btnPreview" value="'.$DiscussionForm->Context->GetDefinition("PreviewPost").'" class="Button SubmitButton PreviewButton" type="button" onclick="ShowPreview(\''.$DiscussionForm->Context->Configuration['BASE_URL'].'extensions/PreviewPost/ajax.php\')" />'
		);
	}
	...
	{
		echo(
			'<input name="btnPreview" value="'.$CommentForm->Context->GetDefinition("PreviewPost").'" class="Button SubmitButton PreviewButton" type="button" onclick="ShowPreview(\''.$CommentForm->Context->Configuration['BASE_URL'].'extensions/PreviewPost/ajax.php\')" />'
		);
	}

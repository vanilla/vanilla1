...
class ConfigurationManager {
   ...
   function UpdateConfigurationFileContents($File) {
      $Lines = @file($File);
      if (!$Lines) {
         $this->Context->WarningCollector->Add($this->Context->GetDefinition("ErrReadFileSettings").$File);
      } else {
         $CurrentLine = "";
         $CurrentSetting = "";
            ...
            if (substr($CurrentLine, 0, 16) == "\$Configuration[\"") {
               ...
               if (count($Values) == 2) {
                  ...
                  if (array_key_exists($CurrentSetting, $this->_settings)) {
                     $Lines[$i] = "\$Configuration[\"".$CurrentSetting."\"] = \"".$this->EncodeSettingValueForSaving($this->_settings[$CurrentSetting])."\"; ".$Comments."\r\n";
         ...
         while (list($Name, $Value) = each($this->_settings)) {
            if (array_key_exists($Name, $this->Settings)) {
               if ($this->Settings[$Name] != $Value) {
                  $Lines[] = "\$Configuration[\"".$Name."\"] = \"".$this->EncodeSettingValueForSaving($this->_settings[$Name])."\";\r\n";
               }               
            }            
         }
         $Lines[] = "?>";

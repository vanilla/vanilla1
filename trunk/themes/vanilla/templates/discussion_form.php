...
<div class="DiscussionForm">'
   ...
   if ($this->Context->Configuration['ENABLE_WHISPERS'] && $Discussion->DiscussionID == 0) {   
      ...
      <dd class="WhisperInput">
         <input id="WhisperUsername" name="WhisperUsername" type="text" value="'.FormatStringForDisplay($Discussion->WhisperUsername, 0).'" class="WhisperBox" maxlength="20" /><div class="Autocomplete" id="WhisperUsername_Choices"></div><script type="text/javascript">'
         ."new Ajax.Autocompleter('WhisperUsername', 'WhisperUsername_Choices', './ajax/getusers.php', {paramName: \"Search\"})"
         .'</script>
      </dd>');
   }
   ...
   echo('<div class="FormButtons DiscussionButtons">
      <input type="submit" name="btnSave" value="'.$this->Context->GetDefinition(($Discussion->DiscussionID > 0) ? 'SaveYourChanges' : 'StartYourDiscussion').'" class="Button SubmitButton" onclick="'

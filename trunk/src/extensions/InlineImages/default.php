...
Description: Display attached images in your comments. Requires Attachments 2
Version: 1.4
Author: Klaus Burton (previously Maurice (Jazzman) Krijtenberg)
...
if (in_array($Context->SelfUrl, array('comments.php'))) {
	...
	$Head->AddScript('extensions/InlineImages/functions.js');
}
	...
	function InlineImages_RenderAttachment(&$AttachmentManager) {
		...
		if( file_exists( $Attachment->Path ) && in_array($Attachment->Extension, array('jpg', 'jpeg', 'gif', 'png', 'bmp')) ) {
			...
			if( strpos($Comment->Body, $ImageTag) > 0 ) {
				...
				$AttachmentBody .= '<div>'.$ImageBody.'</div>';
			}

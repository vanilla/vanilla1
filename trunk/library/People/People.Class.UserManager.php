   ...
   var $Context;			// The context object that contains all global objects (database, error manager, warning collector, session, etc)
         ...
         if (!$NewUser) {
            ...
            $AffectedUser = $this->GetUserById($UserRoleHistory->UserID);
				...
				if ($AffectedUser->PERMISSION_SIGN_IN) {
					$e->BodyText .= str_replace(array("//1", "//2"),
						array($this->Context->Configuration["APPLICATION_TITLE"], $this->Context->Configuration["DOMAIN"].(substr($this->Context->Configuration["DOMAIN"], strlen($this->Context->Configuration["DOMAIN"])-1) == "/" ? "":"/").$this->Context->Configuration["SIGNIN_URL"]),
				...
				if ($this->Context->WarningCollector->Count() == 0) {
					...
					$e->BodyText = str_replace(array("//1", "//2"),
						array($this->Context->Configuration["APPLICATION_TITLE"],
							...
							$this->Context->GetDefinition("PasswordRequestMessage"));
					$e->Send();
				}	
			}
		}
	...
	function SwitchUserPreference($PreferenceName, $Switch) {
		...
		if ($this->Context->WarningCollector->Count() == 0) {
         ...
         $SerializedPreferences = SerializeArray($this->Context->Session->User->Preferences);
			$s = $this->Context->ObjectFactory->NewContextObject($this->Context, "SqlBuilder");
			$s->SetMainTable("User");
